<template>
    <div id="global">
        ...
        <div class="input_global">
            <input v-model="card_name" /><input oninput="this.value=this.value.replace(/[^0-9]/g,'');" maxlength="4" v-for="(item,index) in form_data" v-model="item.value" @click="set_new_value(item.value,index)" @blur="check_new_value(item.value,index)" />
        </div>
        <div class="input_date">
            <input v-model="month" @click="set_new_month(month)" @blur="check_new_month(month)" />
            <input v-model=" days" @click="set_new_days(days)" @blur="check_new_days(days)" />
        </div>
    </div>
</template>
<script>
